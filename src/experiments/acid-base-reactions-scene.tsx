...
export function AcidBaseReactionsSceneComponent({
  ...
  const numAcid = molecules.filter((m: Molecule) => m.type === "acid").length;
  const numBase = molecules.filter((m: Molecule) => m.type === "base").length;
  ...
  useEffect(() => {
    onDataChange?.({
      ...
      numAcid,
      numBase,
      numProduct: molecules.filter((m: Molecule) => m.type === "product").length,
      temperature
    });
  }, [pH, numAcid, numBase, molecules.length, temperature, onDataChange]);
