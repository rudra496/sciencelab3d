...
export default function ElectrolysisScene() {
  ...
  const { voltage, electrolyte } = useControls("Electrolysis", {
    voltage: { value: 5, min: 1, max: 12, step: 0.5, label: "Voltage (V)" },
    electrolyte: {
      value: "water",
      options: ["water", "copper-sulfate"],
      label: "Solution"
    },
  });
  ...
  useFrame((_, delta) => {
    ...
    // Generate new bubbles based on voltage
    if (Math.random() < voltage * 0.02) {
      ...
      setBubbles(prev => [...prev.slice(-100), newBubble]);
    }
